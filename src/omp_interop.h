 ...
 */
int omp_thread_attach(void * new_stack);

/**
 * This function allows call back and flags to return the thread back to user
 * Each time this thread finishes its OpenMP tasks (e.g. finishing a parallel region), the callback_flag
 * will be evaluated. When the flag becomes non-zero, the thread will be removed from the OpenMP runtime
 * by calling the callback routine with the arg argument on the return_stack, if specified as non-NULL.
 */
int omp_thread_attach_with_callback(void * new_stack, int * callback_flag,
                      void *(*callback)(void *), void *arg, void * return_stack);
